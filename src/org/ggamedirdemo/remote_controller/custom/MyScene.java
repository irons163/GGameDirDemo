...
import static com.example.try_gameengine.framework.BitmapUtil.leftKey;
import static com.example.try_gameengine.framework.BitmapUtil.rightKey;
import static com.example.try_gameengine.framework.BitmapUtil.upKey;
import static com.example.try_gameengine.framework.BitmapUtil.downKey;
...
public class MyScene extends EasyScene{
	...
	public MyScene(final Context context, String id, int level, int mode) {
		...
		CustomRemoteController remoteController = CustomRemoteController.createRemoteController();
		setRemoteController(remoteController);
		
		Bitmap leftKeyImage = leftKey;
		Bitmap rightKeyImage = rightKey;
		Bitmap upKeyImage = upKey;
		Bitmap downKeyImage = downKey;
		final Key leftKey = new Key(leftKeyImage, 0, Config.currentScreenHeight-leftKeyImage.getHeight()*2, 1, false);
		leftKey.setEnableMultiTouch(true);
		final Key rightKey = new Key(rightKeyImage, leftKey.getX() + leftKey.getWidth()*2, Config.currentScreenHeight-rightKeyImage.getHeight()*2, 1, false);
		rightKey.setEnableMultiTouch(true);
		final Key upKey = new Key(upKeyImage, leftKey.getX() + leftKey.getWidth() + leftKey.getWidth()/2 - upKeyImage.getWidth()/2, Config.currentScreenHeight-leftKey.getHeight()*2 - leftKey.getHeight()/2 - upKeyImage.getHeight()/2, 1, false);
		upKey.setEnableMultiTouch(true);
		final Key downKey = new Key(downKeyImage, leftKey.getX() + leftKey.getWidth() + leftKey.getWidth()/2 - downKeyImage.getWidth()/2, Config.currentScreenHeight-leftKey.getHeight() + leftKey.getHeight()/2 - downKeyImage.getHeight()/2, 1, false);
		downKey.setEnableMultiTouch(true);
		remoteController.addKey(leftKey);
		remoteController.addKey(rightKey);
		remoteController.addKey(rightKey);
		remoteController.addKey(rightKey);
		final KeyDispatcher keyDispatcher = new KeyDispatcher();
		keyDispatcher.setKeyDispatchListener(new KeyDispatcher.KeyDispatchListener() {
...
//						isMoveing = false;
		...
		remoteController.setRemoteContollerListener(new CustomRemoteController.RemoteContollerListener() {
			
			@Override
			public void pressDown(List<CustomTouch> commandTouchs) {
				// TODO Auto-generated method stub
				
				keyDispatcher.pressDown(commandTouchs);
